...
class Car:
    ...
    def output_to_telegram(self, place, time):
        if time == 'one_week':
            out = f'{place + 1} место\nМарка машины: {self.name_car}\nСреднее количество просмотров в день: {int(self.one_week_pub)}'
        elif time == 'two_week':
            out = f'{place + 1} место\nМарка машины: {self.name_car}\nСреднее количество просмотров в день: {int(self.two_week_pub)}'
        elif time == 'month':
            out = f'{place + 1} место\nМарка машины: {self.name_car}\nСреднее количество просмотров в день: {int(self.month_pub)}'
        return out